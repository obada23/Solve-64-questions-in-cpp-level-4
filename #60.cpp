...
bool isOverlap(stPeriod period, stDate date)
{
    if
    (
        dateCompare(period.endDate,date) == Before
        ||
        dateCompare(period.startDate,date) == After
    )
        return false;
    else
        return true;
...
{system("clear");
    ...
    if(isOverlap(period,date))
        cout << "\nYes, Date is within period" << endl << endl;
    else 
        cout << "\nNo, Date is nit within period" << endl << endl;
